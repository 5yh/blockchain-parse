            ...
            if len(content['logs']) == 0:  # 无log，只取出外部交易
                ...
                try:
                    ...
                    if dict['to'] is None:  # 代表创建合约
                        ...
                        if totype == 'normal':
                            with open('./errorGetcodeAddress2.txt', 'a') as f:
                                f.write(dict['to'] + '\n')
                        totype = 'contract'
...
if __name__ == '__main__':
    ...
    parser.add_argument('-d', '--data', type=str, default="/home/lxl/parse_data1/")  # /pub/p3/test/
    parser.add_argument('-p', '--port', type=str, default="8595")  # /pub/p3/test/
    ...
    for i in range(p_num):
        if i == 0:
            p_list.append(mp.Process(target=parse, args=(i, index[i], index[i + 1], database + 'bnb_parse_' +
                                                         str(index[i]) + '_' + str(index[i + 1]) + '.csv',
                                                         p_num, q, q_count,args.port)))
        else:
            p_list.append(
                mp.Process(target=parse, args=(i, index[i] + 1, index[i + 1], database + 'bnb_parse_' +
