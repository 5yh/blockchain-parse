...
if __name__ == '__main__':
    ...
    parser.add_argument('-d', '--data', type=str, default="/mnt/blockchain2/")  # /pub/p3/test/
    parser.add_argument('-p', '--port', type=str, default="18888")  # /pub/p3/test/
    ...
    for i in range(p_num):
        if i == 0:
            p_list.append(mp.Process(target=parse, args=(i, index[i], index[i + 1], database + 'tron_parse_' +
                                                         str(index[i]) + '_' + str(index[i + 1]) + '.csv',
                                                         p_num, q, q_count,args.port)))
        else:
            p_list.append(
                mp.Process(target=parse, args=(i, index[i] + 1, index[i + 1], database + 'tron_parse_' +
